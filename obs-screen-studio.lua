...
function script_description()
    return "<h2>OBS Screen Studio v" .. VERSION .. "</h2>" ..
...
function do_auto_setup()
    if group_name == nil or group_name == "" then
        obs.script_log(obs.OBS_LOG_WARNING, "[Screen Studio] Set a group name first!")
        return false
    end
    ...
    if group then
        obs.script_log(obs.OBS_LOG_INFO, "[Screen Studio] Group '" .. group_name .. "' already exists. Setup skipped.")
        obs.obs_source_release(scene_source)
        return true
    end
    ...
    if new_group_source then
        ...
        obs.script_log(obs.OBS_LOG_INFO, "[Screen Studio] Auto Setup Complete!")
...
function script_properties()
    ...
    -- ── Support ──────────────────────────────────────────────
    local support_btn = obs.obs_properties_add_button(props, "donate_button", "⭐⭐⭐ SUPPORT THIS SCRIPT ON KO-FI ⭐⭐⭐", function()
        local url = "https://ko-fi.com/kareemstudio"
        if ffi.os == "Windows" then
